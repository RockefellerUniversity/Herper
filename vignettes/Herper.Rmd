...
Unfortunately many tools for data analysis are not available in R, but are present in public repositories like conda. With Herper users can install, manage and run conda tools from the comfort of their R session.
...
condaPaths <- install_CondaSysReqs("HerperTestPkg", pathToMiniConda = myMiniconda)
```
...
pathToSalmon <- file.path(condaPaths$pathToEnvBin,"salmon")
pathToSalmon
system2(cmd=pathToSalmon,args = "-h")
```
...
The **with_CondaEnv** allows users to run R code with the required PATH and environmental variables automatically set. The **with_CondaEnv** function accepts arguments of the Conda environment to use and the code to be executed within the this environment. Additionally we can also the **pathToMiniconda** argument to specify the miniconda install location. 
...
with_CondaEnv("herper",
              system2(cmd="salmon",args = "-h")
              )
```

```{r with_condaenv_SalmonWithCondaEnv, echo=FALSE, eval=TRUE}
if(!identical(.Platform$OS.type, "windows")){
  with_CondaEnv("herper",system2(cmd="salmon",args = "-h"))
}
```
...
with_CondaEnv(new="seqCNA_env", rco <- readSeqsumm(tumour.data=seqsumm_HCC1143)
...
```{r, echo=F, eval=T}
if(!identical(.Platform$OS.type, "windows")){
install_CondaSysReqs("seqCNA",env="seqCNA",pathToMiniConda=myMiniconda)
