...
To demonstrate this we will use the first command from the [seqCNA](https://www.bioconductor.org/packages/release/bioc/html/seqCNA.html) vignette. This step requires samtools. If this is not installed there is an error. 
```{r with_condaenv_R}
library(seqCNA)
...
Samtools is listed as a System Requirement for seqCNA, so we can first use **install_CondaSysReqs()** to install samtools. We can then run the seqCNA command using **with_CondaEnv**, so seqCNA can find samtools. 

```{r, echo=T, eval=F}
install_CondaSysReqs("seqCNA_env",env="seqCNA",pathToMiniConda=myMiniconda)
with_CondaEnv("seqCNA_env", rco <- readSeqsumm(tumour.data=seqsumm_HCC1143)
